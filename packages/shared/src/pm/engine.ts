 ...
 * the phone runs the same rules offline.
 *
 * Two identical copies: apps/api/src/pm/engine.ts (compiled into the API) and
 * packages/shared/src/pm/engine.ts (used by the phone). Edit both; the API test
 * engine-copy.test.ts fails when they differ.
 */
